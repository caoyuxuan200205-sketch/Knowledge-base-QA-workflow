...
答案必须充分利用同一行资料或正文段落中的全部相关要素：当资料行或正文段落提供了名称、年代、材质、尺寸、来源、纹饰等多个要素时，把它们组织成一段连贯完整的答案，而不是只回答单个要素。衔接和组织性表述只能围绕资料内已有信息展开，不得引入任何资料外的内容。
按知识分类组织答案，资料提供了对应信息就应覆盖：
- 文物信息：这是什么 + 年代 + 材质/工艺 + 形制/纹饰 + 价值或看点；
- 展览内容：展览主题 + 内容单元 + 重点展品或亮点；
- 馆务服务：服务内容 + 位置 + 时间/费用 + 使用方式或注意事项；
- 参观政策：政策规定 + 适用对象 + 具体要求 + 例外情况；
- 基建导览：位置 + 到达方式 + 开放情况 + 使用须知。
资料未提供的环节直接省略，不得编造填充。
每个有价值的实体按需要生成0至3个问题，不设最低数量；封面、目录、内部事务或无有效知识的片段返回 {"items":[]}，不得为了凑数量生成问题。
问题必须脱离原文件也能被游客理解，明确指出对象；不使用“根据本文”“该资料”“上文”等指代。问题避免重复。
答案建议长度为120至250字（非强制，资料信息有限时允许更短，不得为凑字数编造内容），要求信息完整、表述自然连贯，把资料中的关键信息点说清楚，不要只给一句话结论。`;
